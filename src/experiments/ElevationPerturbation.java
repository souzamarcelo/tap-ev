 ...
 * @author mdesouza
 */
public class ElevationPerturbation {
    ...
    private void setFixedParameters(String network){
        ...
        Parameters.resultFileName = Parameters.network + "PERTURBATION-ELEVATION";
        ...
        Parameters.perturbElevation = true;
        double[] perturbationValues = {0.0, 1.0, 1.5, 2.0, 4.0};
        Parameters.perturbationValues = perturbationValues;
    ...
    public void run(String network) throws Exception{
        ...
        resultFile.writeContent("method;evaluation;perturbation;% electric;time;energy;gas time;gas energy;elec time;elec energy;runtime [s]");
        
        for(double perturbation : Parameters.perturbationValues){
            Parameters.perturbationValue = perturbation;
            TAPSingleObjective();
            TAPBiobjectiveGE();
            TAPBiobjectiveGS();
        ...
        for(String evaluationMode : evaluationModes){
            ...
            for(double percentageElectric: percentsElectric){
                for(String method: biobjectiveMethods){
                    ...
                    resultFile.writeContent(result.taMethod + ";" + result.energyEvaluationMode + ";" + Parameters.perturbationValue + ";" + result.percentElectricVehicles + ";" + result.timeCost + ";" + result.energyCost + ";" + result.timeOfGasolineVehicles + ";" + result.energyOfGasolineVehicles + ";" + result.timeOfElectricVehicles + ";" + result.energyOfElectricVehicles + ";" + result.executionTime);
                }
            }
            resultFile.writeContent("");
        }
