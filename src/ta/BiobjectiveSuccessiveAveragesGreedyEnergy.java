 ...
 * @author mdesouza
 */
public class BiobjectiveSuccessiveAveragesGreedyEnergy {
        ...
        for(Travel travel: travels){
            ...
            if(amountElectric > 0){
                ...
                for(int i = 0; i < Parameters.admissibleExtraTimeValues.length; i++){
                    ...
                    double lessAgentEnergy = Double.MAX_VALUE;
                    
                    for(Route route: travel.travelRoutes){
                        if(route.timeCost <= (travel.lessTime + (travel.lessTime * (Parameters.admissibleExtraTimeValues[i] / 100d)))){
                            if(acceptableSlope(route.additionalTime, route.energySavings, travel.travelRoute.timeCost, travel.travelRoute.energyCost)){
                                if(chosenRoute == null){
                                    chosenRoute = route;
                                    lessAgentEnergy = route.energyCost;
                                } else {
                                    if(route.energyCost < lessAgentEnergy){
                                        chosenRoute = route;
                                        lessAgentEnergy = route.energyCost;
                                    }
                                }
    ...
    private boolean acceptableSlope(double routeAdditionalTime, double routeEnergySavings, double minIterationTime, double iterationEnergy){
